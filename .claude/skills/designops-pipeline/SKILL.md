...
description: >
  ...
  (audit_prototype.py — 9 static gates: tokens + WCAG contrast in light/dark + no-emoji + component contracts + no remote-font @import + a fidelity family (theme colours + font + non-colour axes) + directive fidelity + screen coverage + edge-case coverage — plus an optional render gate 10: structure/control-parity, needs a build + Playwright) before handoff.
...
TOR (PDF / DOCX / Notion / GDocs)
  ...
  └─────────────┘     └────────┬─────────┘
              ...
              └────────────────────────────────┘
                               ...
                               ▼  Step 4.7  audit gate (9 static gates incl. fidelity family + optional render gate 10: control parity/surface)
...
> …and the **fidelity family** (gate 6) — one gate answering one question: *did the committed Step 2.6 direction actually get applied, or did it regress to the scaffold default?* It runs three sub-checks (each keeps its own linter; the family 🔴 blocks if any sub fails):
> - **theme colours** (`scripts/lint_theme_fidelity.py`): the full identity theme committed in `brand.config.json` (`colors.{light,dark}`) is applied in `globals.css` — if `card`/`secondary`/`muted`/`accent`/`border` or the dark set drifted back to the shadcn-neutral default ("brand colour slapped on a neutral skeleton") it fails. Compares committed hex to the rendered token (oklch→sRGB, same math as gate 2) — deterministic.
> - **font** (`scripts/lint_font_fidelity.py`): the committed `font_sans` primary family (e.g. `Inter` from `"Inter", "Noto Sans Thai", …`) is referenced by `app/layout.*` / `globals.css`, not left at the scaffold default loader (Geist). Gate 5 only forbids a remote `@import`, the theme sub only checks colours — nothing else caught a silent font no-op (a real regression for a Thai TOR committing `Noto Sans Thai`).
> - **non-colour axes** (`scripts/lint_axis_fidelity.py`): the `aesthetic.json` `axes` `resolved` metrics land in `globals.css` — `typography` (line-height/weight via `@theme --text-*`), `shape.pill_slots[]` (`[data-slot=…]{ @apply rounded-full }`), `motion.easing`, and the **layout** tokens (grid gutter/container/control-height in `@theme`). Catches the "declared but not applied" no-op for type/shape/motion/layout. The bridge techniques it expects are no-hardcode-safe (re-point `@theme` in rem/unitless; brand-scoped `[data-slot=*]` rules).
>
> All three follow a LOCAL `@import "./brand.css"` in `globals.css` (inline, cascade-preserving) so the DS-native theming path (`brand.config.json` → `npx ds-brand-build` → `app/brand.css`) verifies; a package specifier (the DS neutral base) is deliberately NOT followed. Each sub auto-discovers its artifact beside the prototype (or `--theme`/`--aesthetic`) and skips cleanly when nothing is committed. (Formerly three separate gates 6/10/11 — folded, since they are one concern.)
